/**
 * MetricChartModel: A charting & graphics component jquery control
 * Whitehead, 2012
 * jquery plugin model inspired by jstree.
 */

(function ($) {
	// private variables 
	var instances = [],			// instance array (used by $.mcmodel.reference/create/focused)
		focused_instance = -1,	// the index in the instance array of the currently focused instance
		plugins = {};			// no plugins yet, but who knows ......
	// init the global settings
	$.fn.mcmodel = function (settings) {
		var isMethodCall = (typeof settings == 'string'), // is this a method call like $().mcmodel("minimize")
		args = Array.prototype.slice.call(arguments, 1), 
		returnValue = this;

		// extend settings and allow for multiple hashes and metadata
		if(!isMethodCall && $.meta) { args.push($.metadata.get(this).mcmodel); }
		...
		if(isMethodCall) {
			this.each(function() {
				var instance = instances[$.data(this, "mcmodel-instance-id")],
					methodValue = (instance && $.isFunction(instance[settings])) ? instance[settings].apply(instance, args) : instance;
					if(typeof methodValue !== "undefined" && (settings.indexOf("is_" === 0) || (methodValue !== true && methodValue !== false))) { returnValue = methodValue; return false; }
			});
		} else {
			this.each(function() {
				var instance_id = $.data(this, "mcmodel-instance-id"),
					s = false;
				// if an instance already exists, destroy it first
				if(typeof instance_id !== "undefined" && instances[instance_id]) { instances[instance_id].destroy(); }
				// push a new empty object to the instances array
				instance_id = parseInt(instances.push({}),10) - 1;
				// store the mcmodel instance id to the container element
				$.data(this, "mcmodel-instance-id", instance_id);
				// clean up all plugins
				if(!settings) { settings = {}; }
				settings.plugins = $.isArray(settings.plugins) ? settings.plugins : $.mcmodel.defaults.plugins;
				if($.inArray("core", settings.plugins) === -1) { settings.plugins.unshift("core"); }
				
				s = $.extend(true, {}, $.mcmodel.defaults, settings);
				s.plugins = settings.plugins;
				$.each(plugins, function (i, val) { if($.inArray(i, s.plugins) === -1) { s[i] = null; delete s[i]; } });
				// push the new object to the instances array (at the same time set the default classes to the container) and init
				instances[instance_id] = new $.mcmodel._instance(instance_id, $(this).addClass("mcmodel mcmodel-" + instance_id), s); 
	...
	$.mcmodel = {
		...
		_reference : function (needle) { 
			...
			return instances[o.closest(".mcmodel").data("mcmodel-instance-id")] || null; 
		},
		...
		plugin : function (pname, pdata) {
			...
			$.mcmodel.defaults[pname] = pdata.defaults;
			$.each(pdata._fn, function (i, val) {
				val.plugin		= pname;
				val.old			= $.mcmodel._fn[i];
				$.mcmodel._fn[i] = function () {
					var rslt,
						func = val,
						args = Array.prototype.slice.call(arguments),
						evnt = new $.Event("before.mcmodel"),
						rlbk = false;
						...
						rslt = func.apply(
							$.extend({}, this, { 
								__callback : function (data) { 
									this.get_container().triggerHandler( i + '.mcmodel', { "inst" : this, "args" : args, "rslt" : data, "rlbk" : rlbk });
								},
								__rollback : function () { 
				...
				$.mcmodel._fn[i].old = val.old;
				$.mcmodel._fn[i].plugin = pname;
			});
		},
	...
	$.mcmodel._fn = $.mcmodel._instance.prototype = {};
	...
	$.mcmodel.plugin("core", {
		...
		defaults : { 
			title_bar	: true,   // css colour is #AABBDD
			css			: {
		    	height: 132px;
	    		width: 132px;
		...
		_fn : { 
			init	: function () { 
				this.set_focus(); 
				this.get_container().html("<div class='mcmodel-last mcmodel-loading'><div class='mcmodel-title'></div>" + this._get_settings().core.strings.loading + "</div>");
